...
def convert_360p(video_path):
    file_name, file_extension = os.path.splitext(video_path)
    new_video_path = file_name + '_360p' + file_extension
    cmd = 'ffmpeg -i "{}" -s hd360 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(video_path, new_video_path)
...
def convert_1080p(video_path):    
    file_name, file_extension = os.path.splitext(video_path)
    new_video_path = file_name + '_1080p' + file_extension   
    cmd = 'ffmpeg -i "{}" -s hd1080 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(video_path, new_video_path)    
    run = subprocess.run(cmd, shell=True)